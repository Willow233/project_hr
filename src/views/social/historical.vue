            ...
            <div class="title">
              ...
              <div class="yearChange">
                <el-date-picker
                  ...
                  @change="changeYear"
                />
                <el-button style="margin-left:10px;" size="mini" type="primary" @click="$router.push('/social_securitys')">返回</el-button>
              </div>
            </div>
          </div>
          <div v-loading="loading" class="historicalTable">
            <div v-for="( itemes, index) in tableData" :key="index" class="itemes">
              <div class="itemTopLab" :class="{act: itemes.act}">
                <div>
                  <p class="itemTit"><span>日期</span></p>
                  <p class="title">{{ itemes.yearsMonth }}</p>
                </div>
                <div>
                ...
                <el-button class="lab" type="text" @click="openTable(itemes,index)">查看报表详情</el-button>
              </div>
  ...
  methods: {
    async getArchivingList() {
      ...
      if (this.tableData.length === 0) {
        this.$message.warning('暂无该年份社保报表')
      }
      this.loading = false
    },
